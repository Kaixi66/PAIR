...
class MLPResNet(nn.Module):
    ...
    @staticmethod
    def _apply_pair_injection(x, pair_init, gate):
        pair_x = pair_init.to(device=x.device, dtype=x.dtype)
        if pair_x.shape != x.shape:
            raise ValueError(f"Expected PAIR delta with shape {tuple(x.shape)}, got {tuple(pair_x.shape)}")
        gate = gate.to(device=x.device, dtype=x.dtype) if torch.is_tensor(gate) else x.new_tensor(gate)
        if gate.shape != x.shape[:2]:
            raise ValueError(f"Expected PAIR gate with shape {x.shape[:2]}, got {gate.shape}")
        return x + gate.unsqueeze(-1) * pair_x
    ...
    def forward(
        ...
        if pair_gate is not None and pair_init is None:
            raise ValueError("PAIR gate was provided without a PAIR delta.")
        if pair_init is not None:
            if pair_gate is None:
                raise ValueError("PAIR delta was provided without its gate.")
            # The canonical PAIR model injects once, immediately after the
            # Action Expert stem and before its first residual block.
            x = self._apply_pair_injection(x, pair_init, pair_gate)
