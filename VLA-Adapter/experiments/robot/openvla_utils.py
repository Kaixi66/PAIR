...
try:
    from prismatic.models.pair_bridge import load_pair_bridge_checkpoint
except ImportError:
    load_pair_bridge_checkpoint = None
...
EVAL_CONFIG_FILENAME = "vla_adapter_eval_config.json"
...
def find_checkpoint_files(pretrained_checkpoint: str, file_pattern: str) -> List[str]:
    """Find checkpoint files matching a component pattern."""
...
def load_eval_config_from_checkpoint(pretrained_checkpoint: str) -> Dict[str, Any]:
    """
    Load eval-time model wiring from a checkpoint.

    New checkpoints store this explicitly. For older local checkpoints, infer only
    component presence; do not guess architecture values that are not represented
    on disk.
    """
    if model_is_on_hf_hub(pretrained_checkpoint) or not os.path.isdir(pretrained_checkpoint):
        return {}

    eval_config_path = os.path.join(pretrained_checkpoint, EVAL_CONFIG_FILENAME)
    if os.path.exists(eval_config_path):
        with open(eval_config_path, "r") as f:
            payload = json.load(f)
        return payload.get("model_config", payload)

    return {
        "use_l1_regression": checkpoint_has_component(pretrained_checkpoint, "action_head"),
        "use_proprio": checkpoint_has_component(pretrained_checkpoint, "proprio_projector"),
        "use_film": checkpoint_has_component(pretrained_checkpoint, "vision_backbone"),
        "use_pair_bridge": checkpoint_has_component(pretrained_checkpoint, "pair_bridge"),
    }
...
def get_action_head(cfg: Any, llm_dim: int) -> Union[L1RegressionActionHead]:
    ...
    if getattr(cfg, "use_pro_version", None) is None:
...
def get_pair_bridge(cfg: Any, llm_dim: int) -> Optional[torch.nn.Module]:
    """Load a PAIR bridge checkpoint for inference when the checkpoint contains one."""
    if model_is_on_hf_hub(cfg.pretrained_checkpoint):
        if getattr(cfg, "use_pair_bridge", False):
            raise ValueError("PAIR bridge loading from Hugging Face Hub checkpoints is not supported in this local build.")
        return None

    has_pair_bridge = checkpoint_has_component(cfg.pretrained_checkpoint, "pair_bridge")
    if not has_pair_bridge:
        if getattr(cfg, "use_pair_bridge", False):
            find_checkpoint_file(cfg.pretrained_checkpoint, "pair_bridge")
        return None

    cfg.use_pair_bridge = True
    if load_pair_bridge_checkpoint is None:
        raise ImportError(
            "PAIR bridge module is not importable. Make sure VLA-Adapter is on PYTHONPATH."
        )

    checkpoint_path = find_checkpoint_file(cfg.pretrained_checkpoint, "pair_bridge")
...
def get_vla_action(
    ...
    pair_bridge: Optional[torch.nn.Module] = None,
    use_film: bool = False,
