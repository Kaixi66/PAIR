...
# Example:
#   PRETRAINED_CHECKPOINT=/path/to/checkpoint TASK_SUITES=libero_spatial ./eval_libero_vla_adapter.sh
...
source "${ENV_SH}"
REPO_DIR="${REPO_DIR:-${PAIR_REPO_DIR}}"
LOG_DIR="${LOG_DIR:-${PAIR_LOGS_DIR}/eval}"
...
[[ -n "${PRETRAINED_CHECKPOINT}" ]] || {
    echo "PRETRAINED_CHECKPOINT is required." >&2
    exit 2
}
